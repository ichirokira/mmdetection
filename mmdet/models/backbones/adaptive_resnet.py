...
class MlpMixer(nn.Module):
    def __init__(self, img_size, hidden_dim=768, tokens_mlp_dim=384, channels_mlp_dim=3072,num_blocks=2,patch_size=16, zero_head=False):
        ...
        for _ in range(num_blocks):
            layer = MixerBlock(n_patches, tokens_mlp_dim, channels_mlp_dim, hidden_dim)
...
class LP_Modules(nn.Module):
    def __init__(self, patch_size, image_sizes, hidden_dim=768, tokens_mlp_dim=384, channels_mlp_dim=3072,num_blocks=12
                 , zero_head=False):
        """
    ...
    def forward(self, x1, x2):
        ...
        self.kernel = self.kernel.view(B, 1, 1, 1, -1)
...
class AdaptiveResNet(BaseModule):
    """Adaptive ResNet backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(AdaptiveResNet, self).__init__(init_cfg)
        ...
        self.adaptive_layer = self.make_adaptive_layers()
    ...
    def make_adaptive_layers(self):
        return LP_Modules(patch_size=self.adaptive_kernel, image_sizes=self.img_size, hidden_dim=self.hidden_dim,
                          tokens_mlp_dim=self.tokens_mlp_dim, channels_mlp_dim=self.channels_mlp_dim, num_blocks=self.num_blocks
                          , zero_head=self.zero_head)
    ...
    def forward(self, x, lp_x):
        """Forward function."""
        x = self.adaptive_layer(x, lp_x)
        if self.deep_stem:
            x = self.stem(x)
        else:
            x = self.conv1(x)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(AdaptiveResNet, self).train(mode)
        self._freeze_stages()
